...
describe('toHaveNoViolations', () => {
    it('passes for an empty result', () => {
        const result = toHaveNoViolations.call(matcherContext, []);

        expect(result).toMatchObject({ pass: true });
        expect(result.message()).toContain('Expected the DOM to contain at least one Contrast Lens finding.');
    });

    it('throws when the received value is not a Finding[]', () => {
        expect(() => toHaveNoViolations.call(matcherContext, 'not-an-array' as never)).toThrow(TypeError);
    });
